...
5) Many papers make changes to the training process that could be applied to a whole family of algorithms (e.g. MBPO adds model-based data generation to any off-policy model-free algorithm). To make this easy to implement, each algorithm's gradient update step is separated from it's training loop. So, for example, MBPO can set up it's own training loop, and then call TD3 or SAC's learning update with very few changes.

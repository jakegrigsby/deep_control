...
def learn(
    ...
    with torch.no_grad():
        ...
        logp_cema1 = action_dist_s1.log_prob(cem_action_s1).sum(-1, keepdim=True)
    ...
    learning_info = {
            ...
            'y1':y_1.mean(),
            'better_action_mask_sum':better_action_mask.sum(),
            'logp_a1_mean':logp_a1.mean(),
            'logp_a1_max':logp_a1.max(),
            'logp_a1_min':logp_a1.min(),
            'logp_cema1_mean':logp_cema1.mean(),
            'logp_cema1_max':logp_cema1.max(),
            'logp_cema1_min':logp_cema1.min(),
            'clip_double_q_a1_mean':clip_double_q_a1.mean(),
            'clip_double_q_cema1_mean':clip_double_q_cema1.mean(),
        }
    ...
    cem_action_value = torch.min(cem_action_value_q1, cem_action_value_q2)
    use_q1_actions_mask = (cem_action_value_q1 <= cem_action_value_q2).squeeze(1)
    cem_actions = cem_actions_q2.clone()
    cem_actions[use_q1_actions_mask] = cem_actions_q1[use_q1_actions_mask]
    ...
    learning_info.update({'alpha_loss':alpha_loss})
    """
...
def add_args(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help="Learning rate for entropy coefficient parameter (as in SAC)",
    )
    parser.add_argument(
        "--init_alpha",
        type=float,
        default=.1,
        help="Initial value of entropy coefficient (as in SAC)",
    )
